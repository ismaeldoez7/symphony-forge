...
status: approved
saved: 2026-09-11T20:04:57+00:00
story: FORGE-COORD-1
decisions_reviewed:
...
| `lean-delivery-graph.json` | `dc065611969c13f9537b7ed5b42e500094927f0c286fdc857fc403198e5a969b` | AC/roadmap/owner crosswalk plus the explicit new model-policy and combined-review allocation annexes plus `prepared_hunk_owners`: all 40 original paths and 201 hunk identities, hashes and allocations. |
...
First owns an amended 75-path task envelope: the live 66-path contract remains intact and adds `factory/scripts/record_review_from_json.py`, `factory/schemas/review-set.json`, `docs/product/BRIEF.md`, `docs/architecture/dual-coordinator-parity.md`, `docs/specs/strict-role-split.md`, `docs/degraded-mode.md`, `factory/tests/test_review_lenses_in_parallel.py`, `factory/tests/test_proof_read_path.py`, and `factory/scripts/forge_cli/close.py`. That total envelope measures and reviews all prior committed First work; it is not permission to revisit it. The one remaining registered worker may edit only the following 16 existing paths plus the one new schema, with at most 2,500 added-plus-deleted lines: `docs/QUALITY.md`, `factory/scripts/check_task_proof.py`, `factory/scripts/factory_lib.py`, `factory/scripts/forge.py`, `factory/scripts/record_review_from_json.py`, `factory/scripts/forge_cli/close.py`, `factory/scripts/forge_cli/review.py`, `factory/scripts/forge_cli/review_brief.py`, `factory/scripts/forge_cli/stages.py`, `factory/scripts/forge_cli/tasks.py`, `factory/tests/test_gates.py`, `factory/tests/test_proof_read_path.py`, `factory/tests/test_review_lenses_in_parallel.py`, `factory/tests/test_review_settled_contracts.py`, `factory/tests/test_review_task_delta.py`, `harness.yaml`, plus new `factory/schemas/review-set.json`. Every other in-envelope path is preserve-byte-for-byte input to this delegate. It stops on any other edit or larger remaining delta and returns the contradiction for amendment. The task-level 180-file/18,000-line review budget remains only the seal-time ceiling over the entire accumulated First task delta and grants no additional edit authority.
