...
Codex executes exploration, implementation, testing, and the review: the orchestrating session releases ONE three-lens autoreview pass per task with `./forge review <task-id>` (the autoreview skill run by Codex — never a nested Codex companion job, 0011; recorded as that task's proof, 0049), watches it, and loops it until clean, delegating any fixes back to Codex. `./forge delegate` is the sole normal write path; a five-file `forge mode degraded` window is the ledgered outage exception. The `.factory` artifacts are required in either route.
...
6. review, once per task when implementation is complete and verified: `./forge review <task-id>` releases Codex for the three lenses (quality, performance, security) and records them as the task's proof (0049), then LOOP — delegate any fixes to Codex, re-review — until every lens is clean
...
- Review = ONE three-lens autoreview pass PER TASK, run by Codex through the autoreview skill and released by the orchestrating session via `./forge review <task-id>` (0011, 0049), looped until clean: review → delegate fixes to Codex → re-review; never a nested Codex companion job, never nested reviewers; recorded before `pr-ready`.
