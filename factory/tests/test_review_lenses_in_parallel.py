...
assert "### Approved task inputs" in open(dataset, encoding="utf-8").read()
...
def _built(repo: Path, tmp_path: Path) -> None:
    """A started stage with committed work and complete task-owned inputs."""
    ...
    automated = {
        ...
        "summary": "parallel review fixture passed", "blocking_findings": [],
    ...
    for name, body in (
        ...
        path = lib.proof_path(
            repo, "ENG-1", name, task_id="T1", for_write=True,
        )
