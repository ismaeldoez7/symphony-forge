...
def cmd_doctor(args: argparse.Namespace) -> None:
    ...
    # It ships in Matt Pocock's skills pack, so --fix installs the pack for the
    # coordinator's global use; both runtimes ALSO load the repo-vendored copy
    # (.claude/skills/grill-me + .codex/skills/grill-me), so the Codex delegate
    # needs no HOME mirror.
    ...
    # ponytail (the minimal-diff coding discipline every code writer/editor
    # follows) needs no doctor install: it is vendored into the repo at
    # .claude/skills/ponytail + .codex/skills/ponytail, so both runtimes load it
    # from every worktree and the implementer brief inlines the same text.
