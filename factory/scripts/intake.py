...
def main(argv: list[str] | None = None) -> None:
    ...
    if outcome == "absent":
        # Naming `roadmap add` unconditionally sent a pre-sign-off agent to a
        # command that refuses as "post-sign-off grooming" — a refusal pointing
        # at a blocked command. Say which gate is actually in the way.
        ...
        if not client_signoff(root)[0]:
            raise SystemExit(
                ...
                "them (`forge roadmap derive --input <file>`), grill and accept "
