...
summary, `forge next` surfaces the count — and on real activity: the
`harness-health` workflow runs it in CI on `push:main` and manual
`workflow_dispatch`, not a daily cron. ADVISORY, never a ship gate.
