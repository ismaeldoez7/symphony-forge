...
The recorded 76-path scope and 180-file/18,000-line review budget measure the complete accumulated First task, including already committed work. They do not authorize this delegate to revisit those bytes. This delegate may edit only these 17 existing paths plus the one new schema, with at most 3,200 added-plus-deleted lines:
...
Every other path in the 76-path task envelope is preserve-byte-for-byte input. Stop and report a contract amendment if another path or more than 3,200 changed lines is mechanically required.
...
Before implementation, Main records the amended 76-path/61-test decomposition and saves, grills, and approves this exact plan. One registered `gpt-5.6-sol`/medium worker then writes the tests first, demonstrates the new integrity, canonical-marker, and selected-generation fixture cases fail against the current runtime, applies the smallest production change, and runs the twenty-two-selector focused batch. The worker finishes with `./forge next` and the real harmless native PreToolUse denial probe. Main correlates that public terminal evidence, commits the product delta, records required proof, and runs the resumable `forge task close` path. A blocking review returns as one bounded Sol/medium fix batch; the next close attempt repeats only proof made stale by the new delta.
...
Origins add one mutually exclusive field. `combined` forbids `rejection` and `upgrade`. `rejection` requires exactly `{source_generation_id, source_generation_sha256, root_generation_id, history}`; its source is the selected `origin=combined` generation, `root_generation_id` equals that source ID, and `history` contains exactly one `{finding_fingerprint, reason, citation, actor}` entry. `upgrade` requires exactly `{inventory_digest, source_kind, legacy_artifacts, sealed_commit}` where `source_kind` is `active` or `sealed`, `legacy_artifacts` is the sorted exact `{aspect, path, sha256}` triple, active uses an empty `sealed_commit`, and sealed requires the exact marker commit. The selected pointer is exactly `{format: forge-review-selection/v1, story, task_id, generation_id, generation_sha256, delta_id, selected_at}`. `generation_sha256` always means SHA256 of the exact deterministic generation-file bytes; a rejection's `source_generation_sha256` means that same file-byte hash, removing any ambiguous “source hash.”

`python3 factory/scripts/record_review_from_json.py --set --task <id> --input <candidate.json>` records only `origin=combined` and is mutually exclusive with diagnostic `--aspect`. The candidate supplies every generation field except `generation_id`; the recorder resolves the documented active-task default, verifies the current task token/delta, decodes the raw helper report, rederives all three lens records from authoritative task state, and refuses any caller-supplied lens mismatch before it derives the ID, publishes, reads back, and selects. Citation rejection uses `forge review --reject`; the future Portable whole-inventory upgrade uses the same internal schema-backed publication function only after its preflight.
...
`--reject --lens <lens>` starts only from the selected `origin=combined` generation. It writes one immutable `origin=rejection` successor bound to that source generation file SHA256 and ID, plus the exact finding, reason, citation, and actor. It preserves the exact decoded `raw_result` bytes, ordered passes, shared bindings, and unaffected lenses, then publishes the successor pointer last. Rejection, fixed-only, upgrade, unselected, or unrelated sources refuse.
...
- `factory/tests/test_review_settled_contracts.py::test_reject_republishes_one_complete_pointer_selected_set` — require a selected combined source, preserve the exact cited successor, and refuse rejection, upgrade, unselected, or unrelated sources.
...
- `factory/tests/test_gates.py::test_quality_review_requires_contract_verdicts`
...
The new integrity, canonical-marker, active-task-default, and selected-generation fixture assertions must be RED against the current runtime before production edits. A selector that stays green means the diagnosis or test is wrong; correct the test rather than changing production blindly. After the focused batch passes, the worker runs the three recorded verify commands unchanged.

## Manual Verification

1. Run the focused command and confirm exactly twenty-two selected tests pass with a fresh JUnit report.
...
**Objective.** Act as the registered gpt-5.6-sol/medium worker for approved NATIVE-FOREGROUND-ACTIVATE. Read the protected task plan and current tree, use ponytail, and edit only its named 18-path remaining-delegate subset within 3,200 lines; preserve every other accumulated First path byte-for-byte and never replay historical patches. Implement complete D-0032 with one helper call per complete review. Publish one content-addressed immutable raw-plus-three-lens generation, then the task selected.json pointer last. Derive generation_id from canonical sorted compact UTF-8 JSON excluding generation_id; recompute it on read, require real non-symlink directory ancestors and regular single-link leaves, accept byte-identical retries, and refuse unequal collisions. Validate exact ordered full-line BEGIN/END FORGE ASSESSMENT markers, chunk order, one lens tag per finding, normalized path/line/title duplicate identity, existing scores/recommendations, and worst quality verdicts; rederive combined lenses from raw bytes before publication. Reuse product_excluded_prefixes/product_delta_digest so review and task close share one delta_id. Rejection extends only the selected combined generation, binds its source file SHA256 and ID, and preserves exact decoded raw-result bytes and unaffected lenses. Diagnostics cannot publish, stamp, revoke, or compete with selected proof. Failed, stale, copied, mixed, fixed-only, story-fallback, or interrupted proof leaves the pointer unchanged. Rewrite the inherited parallel-review and story-fallback tests, add the one schema, run the twenty-two-selector focused batch, then run forge next and the harmless real protected-plan denial probe. Return concise prose; Main owns correlation, commit, proof, closeout, PR, and CI. Treat origin/main c9a705bc closeout/delta/proof/board/CI as inherited baseline.
