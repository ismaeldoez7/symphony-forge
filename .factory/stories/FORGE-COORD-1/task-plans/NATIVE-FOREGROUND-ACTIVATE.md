...
3. Make combined finding projection distinguish a missing `source_attribution` key from the required explicit `null`; plain-source output without the key refuses. Update valid helper-shaped fixtures to include the explicit null and extend the existing malformed-output selector with key deletion.
4. Restrict raw `codex exec` detection to actual executable positions reached through the already-supported literal wrappers and shell command operands. Preserve denial for the existing direct, `command`, `env`, `nohup`, `nice`, `xargs`, and shell-wrapper cases, while allowing argument data after an unrelated executable such as `python audit.py codex exec` and `tool --label codex exec`. Do not turn this into a general shell parser or weaken substitution/pipeline denial.
...
**Objective.** Act as the registered gpt-5.6-sol/medium seventh review-follow-up worker for NATIVE-FOREGROUND-ACTIVATE under the incumbent 77-path scope plus its protected test_task_parallelism.py amendment (78 effective paths) and 18,000-line seal ceiling. Use ponytail. Starting after committed sixth remediation 9472dc8, change only AGENTS.md, factory/scripts/forge_cli/worker_admission.py, factory/scripts/forge_cli/review.py, factory/scripts/pre_tool_use.py, factory/tests/test_worker_admission.py, factory/tests/test_review_task_delta.py, factory/tests/test_native_setup.py, and factory/tests/test_gates.py, within 260 added-plus-deleted lines from Main's exact new pre-delegation HEAD. Resolve the six selected review P2 findings: document selected-generation proof authority; require, lifecycle-bind, task-match, baseline-classify, and argv-validate the native row's own recorded write_scope; require an explicit null source_attribution key on plain-source findings; limit wrapped raw-Codex matching to actual executable positions while preserving all supported wrapper denials; assert malformed hook matchers return unhealthy readiness; and prove missing, malformed, changed, or mismatched recorded scope never grants live admission. Extend existing selectors only. Do not add a parser abstraction beyond the existing literal wrapper seam, weaken raw launch denial, change doctor.py unless the strengthened assertion is RED, add a file/selector/dependency/schema/task/criterion/scope entry, or touch the selected review generation. Preserve the correlated C8 denial and every other prior First-task behavior and proof. Main owns exact affected selectors, the native suite, all 62 selectors, encoding, full verifier, evidence, review, closeout, PR, and CI.
...
**Review budget.** 180 files / 18000 lines -- Seal-time ceiling over the complete accumulated First task delta: the 77-path recorded envelope plus the existing protected factory/tests/test_task_parallelism.py amendment (78 effective paths). It grants no launch-specific subset. Main records the exact new pre-delegation HEAD and accepts this seventh review-follow-up batch only when its delta is AGENTS.md, factory/scripts/forge_cli/worker_admission.py, factory/scripts/forge_cli/review.py, factory/scripts/pre_tool_use.py, factory/tests/test_worker_admission.py, factory/tests/test_review_task_delta.py, factory/tests/test_native_setup.py, and factory/tests/test_gates.py and no more than 260 added-plus-deleted lines.
<!-- /forge:contract -->
