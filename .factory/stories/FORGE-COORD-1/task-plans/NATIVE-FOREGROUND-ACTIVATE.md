# NATIVE-FOREGROUND-ACTIVATE — correct the exec candidate

Correct only the incomplete raw-`exec` candidate left by the focused worker. Restore `_wrapped_codex_exec` exactly to `c462478`; shell-builtin `exec` belongs only in the quote-aware active-boundary matcher. Use the bounded `command`/`exec` prefix grammar already validated against the full detector matrix, including real `-c`/`-l`/`-a` forms. Keep the completed unconditional symlink setup. Main retains recorder, Git, review, close, PR, and CI ownership.

## Workflow

```mermaid
flowchart LR
    A[Audit failed candidate] --> B[Restore argv wrapper parser]
    B --> C[Complete active-boundary regex]
    C --> D[Exact finite positive and negative matrix]
    D --> E[Two focused selectors and diff check]
    E --> F[Restore full close measurement]
```

1. Remove `exec_codex_at`, the `name == "exec"` arm, and the compensating `nohup`/`env`/`nice`/`xargs` guards so `_wrapped_codex_exec` matches `c462478` exactly.
2. At the active-boundary regex, use `COMMAND_PREFIX = r'(?:command(?:\s+-p)*\s+(?:--\s+)?)?'`, quoted or bare bounded `exec`, and `EXEC_OPTION = rf'(?:-[cl]+|-[cl]*a(?:{SHELL_WORD}|\s+{SHELL_WORD}))'`.
3. Add the exact allowed argv0 cases `exec -a 'codex exec' printf '%s' safe`, `exec -cla codex exec x`, and `exec -acl label codex exec x`. Add active semicolon/pipeline cases for attached quoted, mixed, and backslash argv0; include `command -p [--] exec`, substitution, nested shell, wrapper data, and `/tmp/exec` controls.
4. Keep the repair bounded to `pre_tool_use.py`, `test_gates.py`, and the already-correct `test_worker_admission.py`. Run only the existing raw-launch selector, existing symlink selector, and `git diff --check`.

## Manual Verification

1. Confirm `_wrapped_codex_exec` is byte-equivalent to `c462478` and contains no `exec` wrapper arm or compensating wrapper exclusions.
2. Run the raw selector and confirm the full existing matrix plus exact new boundary cases pass.
3. Run the symlink selector and confirm both rows execute without a skip.
4. Run `git diff --check`; confirm only the three authorized paths differ from `c462478` and the four-path cumulative repair remains within 240 changed lines.
...
**Objective.** Correct only the incomplete raw-exec candidate left by the focused worker. Restore _wrapped_codex_exec exactly to c462478 and implement bounded shell-builtin exec handling solely in the quote-aware active-boundary matcher, including command -p/-- and real -c/-l/-a forms, exact argv0/data negatives, later separators/pipelines, substitutions, nested shells, wrapper data, and external /tmp/exec. Keep the unconditional symlink regression fix. Change only factory/scripts/pre_tool_use.py, factory/tests/test_gates.py, and factory/tests/test_worker_admission.py; Main owns recorders, Git, review, close, PR, and CI.
