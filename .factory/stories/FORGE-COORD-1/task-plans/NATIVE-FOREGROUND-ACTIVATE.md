...
Before implementation, Main records the amended 77-path/62-test decomposition and existing protected scope amendment, saves, grills, and approves this exact plan while preserving the already-active stage incarnation. Main refreshes the protected incomplete-stage note through the owning recorder and records the exact current pre-delegation HEAD, then launches one ordinary registered `gpt-5.6-sol`/medium worker under the effective 78-path authority. For this third remediation, that worker owns only the eight paths named above and extends existing selectors instead of adding a test module or selector. It runs the focused non-process selector batch that its managed environment supports and performs the harmless real native PreToolUse denial probe. Main validates the eight-path/600-line delta from that exact pre-delegation HEAD and the unchanged complete-task budgets before accepting the contribution, runs `./forge next`, all 62 declared selectors, and every process-heavy/full verification command in the permissive parent environment, correlates the public terminal evidence, commits the product delta, records required proof, and runs the resumable `forge task close` path. A blocking review returns as one bounded Sol/medium fix batch; the next close attempt repeats only proof made stale by the new delta. No degraded window is opened unless a real ordinary-delegation outage is observed and recorded.
...
**Objective.** Act as the registered gpt-5.6-sol/medium worker for NATIVE-FOREGROUND-ACTIVATE under its incumbent 77-path recorded scope plus the existing protected factory/tests/test_task_parallelism.py amendment (78 effective paths) and 18,000-line seal ceiling. Use ponytail. For the third post-integration formal-review remediation, change only factory/scripts/factory_lib.py, factory/scripts/forge_cli/delegate.py, factory/scripts/pre_tool_use.py, factory/tests/test_gates.py, factory/tests/test_native_launch.py, factory/tests/test_proof_read_path.py, factory/tests/test_review_settled_contracts.py, and factory/tests/test_task_parallelism.py, with no more than 600 added-plus-deleted lines from Main's exact pre-delegation HEAD. Fix the six validated blockers: exact sealed-upgrade pointer binding, non-object proof routing through one central proof-specific typed-read rule across every existing consumer, locale-independent UTF-8 native stdin, whitespace-separated shell-option operands, complete rejection lineage/refusals, and real-process seal/publication exclusion. Reuse the existing 62 selectors and preserve the complete Decision 0069 one-helper generation, Decision 0066 closeout, Decision 0070 model policy, prior proof, and every other byte. Use one ordinary delegate and no fabricated degraded outage. Main runs full verification, proof, review, closeout, PR, and CI.
...
**Review budget.** 180 files / 18000 lines -- Seal-time ceiling over the complete accumulated First task delta: the 77-path recorded envelope plus the existing protected factory/tests/test_task_parallelism.py amendment (78 effective paths). It grants no launch-specific subset. Main records the exact pre-delegation HEAD and accepts this third formal-review remediation only when its delta stays within the eight named existing paths and 600 added-plus-deleted lines.
<!-- /forge:contract -->
