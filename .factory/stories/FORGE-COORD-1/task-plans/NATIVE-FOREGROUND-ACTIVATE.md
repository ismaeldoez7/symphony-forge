...
Before the fourth fix batch, Main records the amended 77-path/62-test decomposition and existing protected scope amendment, saves, grills, and approves this exact plan while preserving the already-active stage incarnation. Main refreshes the protected incomplete-stage note through the owning recorder and records the exact current pre-delegation HEAD, then launches one ordinary registered `gpt-5.6-sol`/medium worker under the effective 78-path authority. That worker owns only the six paths named above, extends existing selectors, runs the focused tests its managed environment supports, and performs the real protected-plan denial probe. Main validates the six-path/500-line delta from that exact pre-delegation HEAD, verifies the denial correlation and unchanged bytes against the durable terminal and tool records, runs `./forge next`, all 62 declared selectors, and every process-heavy/full verification command in the permissive parent environment, records current proof, and reruns the complete three-lens review. A blocking review returns as another bounded Sol/medium fix batch; clean proof continues through resumable `forge task close`, PR, and CI. No degraded window is opened unless a real ordinary-delegation outage is observed and recorded.
...
**Objective.** Act as the registered gpt-5.6-sol/medium review-fix worker for NATIVE-FOREGROUND-ACTIVATE under the incumbent 77-path scope plus its protected test_task_parallelism.py amendment (78 effective paths) and 18,000-line seal ceiling. Use ponytail. Starting after committed third remediation bf7a00e, change only factory/scripts/factory_lib.py, factory/scripts/forge_cli/stages.py, factory/scripts/forge_cli/review.py, factory/scripts/forge_cli/doctor.py, factory/tests/test_worker_admission.py, and factory/tests/test_gates.py, within 500 added-plus-deleted lines from Main's exact new pre-delegation HEAD. Detect post-seal proof rewrites; refuse launch rows without recorded write_scope; accept schema-required null plain-source attribution; prove invalid hook regexes return unhealthy readiness without raising and change doctor.py only if that regression is RED; replace the seal/publication fixed delay with a lock-seam handshake; and produce the live launch/session/tool-call-correlated protected-plan denial with unchanged hashes. Extend the existing 62 selectors only. Preserve all prior First-task behavior and proof. Main owns full verification, evidence recording, three-lens review, closeout, PR, and CI.
...
**Review budget.** 180 files / 18000 lines -- Seal-time ceiling over the complete accumulated First task delta: the 77-path recorded envelope plus the existing protected factory/tests/test_task_parallelism.py amendment (78 effective paths). It grants no launch-specific subset. Main records the exact new pre-delegation HEAD and accepts this fourth formal-review fix loop only when its delta stays within the six named existing paths and 500 added-plus-deleted lines.
<!-- /forge:contract -->
