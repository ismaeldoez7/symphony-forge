# NATIVE-FOREGROUND-ACTIVATE — finish the group boundary correction

Preserve the nine-file implementation and its five green focused JUnit reports. Correct only the two remaining audit details: replace the incomplete helper with the approved bounded `(` boundary and update the stale CI proof docstring. Main will restore and run the full 70-selector and five-command contract after this worker returns.

## Workflow

```mermaid
flowchart LR
    A[Preserve nine-file diff] --> B[Use approved group boundary]
    B --> C[Add function-body group case]
    C --> D[Correct proof docstring]
    D --> E[Run raw selector and diff check]
    E --> F[Return to Main for full close proof]
```

1. In `factory/scripts/pre_tool_use.py`, replace `active_group_boundary` with the approved bounded regex alternative `(?<![\w=(@?!+*$])\(\s*`. Keep the worker's parenthesis-depth stack, quote/escape/nested-substitution/backtick ordering, complete exec grammar, and `_wrapped_codex_exec` byte-equivalent to `c462478`.
2. In `factory/tests/test_gates.py`, add the literal `f(){(codex exec x);}; f` raw-launch case and the same grouped function body inside `$()` to the existing deny matrix. Preserve all existing positive and negative controls.
3. In `factory/scripts/check_task_proof.py`, replace the stale legacy-fallback docstring wording with the selected task-proof path and explicit marker-bound upgrade exception. Change no proof behavior.
4. Run only `test_codex_exec_ban_matches_invocations_not_prose` and `git diff --check`. Do not run the inherited full task suite or verify commands; Main owns the restored cumulative close proof.

## Manual Verification

1. Confirm only `factory/scripts/pre_tool_use.py`, `factory/tests/test_gates.py`, and `factory/scripts/check_task_proof.py` change from the preserved worker diff.
2. Confirm the raw selector denies the function-body group at top level and inside `$()`, while the existing array/function/arithmetic/extglob-like data remains allowed.
3. Confirm no `active_group_boundary` helper remains, the CI proof docstring describes selected proof plus marker-bound upgrade, `_wrapped_codex_exec` still matches `c462478`, and `git diff --check` passes.
...
**Objective.** Finish the preserved nine-file review-fix candidate by changing only factory/scripts/pre_tool_use.py, factory/tests/test_gates.py, and factory/scripts/check_task_proof.py. Remove the incomplete active_group_boundary helper and put the exact bounded `(?<![\w=(@?!+*$])\(\s*` alternative directly in CODEX_EXEC_INVOCATION; keep the existing parenthesis-depth frames and all quote, escape, nested-substitution, backtick, exec-option, and wrapper behavior. Add `f(){(codex exec x);}; f` and its nested `$()` counterpart to the raw-launch deny matrix. Replace the CI proof docstring legacy-fallback claim with selected task proof and the marker-bound upgrade exception. Run only test_codex_exec_ban_matches_invocations_not_prose and git diff --check; Main owns restoration of the 70-selector and five-command close contract.
...
**Review budget.** 180 files / 18000 lines -- The cumulative task remains inside the approved 180-file/18,000-line seal ceiling. Relative to 499c358, the final formal-review continuation may change only ten product/test paths: the nine current correction paths plus the prior factory/tests/test_worker_admission.py symlink regression, with at most 380 added-plus-deleted lines. The current worker changes exactly the nine correction paths and leaves test_worker_admission.py untouched. Exclude only recorder-owned .factory/** state; no architecture, decision, dependency, schema, migration format, skip, or unrelated product path may change.
<!-- /forge:contract -->
