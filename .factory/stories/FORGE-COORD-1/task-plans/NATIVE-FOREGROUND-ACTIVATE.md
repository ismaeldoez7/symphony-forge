...
**Objective.** Act as the registered gpt-5.6-sol/medium verification-fix worker for NATIVE-FOREGROUND-ACTIVATE under the incumbent 77-path scope plus its protected test_task_parallelism.py amendment (78 effective paths) and 18,000-line seal ceiling. Use ponytail. Starting after committed fourth remediation 9a299f1, change only factory/tests/test_native_launch.py, within 40 added-plus-deleted lines from Main's exact new pre-delegation HEAD. Update the two existing successful native terminal fixtures to record the task's immutable write_scope and construct argv from that same scope. Do not restore the removed missing-scope fallback, add a selector, or weaken production validation. Preserve the correlated C8 denial and every other prior First-task behavior and proof. Main owns the exact native suite, all 62 selectors, full verifier, evidence recording, three-lens review, closeout, PR, and CI.
...
**Review budget.** 180 files / 18000 lines -- Seal-time ceiling over the complete accumulated First task delta: the 77-path recorded envelope plus the existing protected factory/tests/test_task_parallelism.py amendment (78 effective paths). It grants no launch-specific subset. Main records the exact new pre-delegation HEAD and accepts this fifth verification-fix batch only when its delta is factory/tests/test_native_launch.py and no more than 40 added-plus-deleted lines.
<!-- /forge:contract -->
