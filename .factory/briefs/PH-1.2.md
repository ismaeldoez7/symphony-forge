# Brief — PH-1.2: Sign-off refuses an incomplete brief
...
Extend record_signoff.workflow_input_problems so a brief missing a required heading, or carrying one with an empty body, refuses sign-off and names the heading.

## Acceptance criteria

- record_signoff.py refuses when docs/product/BRIEF.md is absent, missing a required heading, or has a required heading whose body is empty.
- The refusal names every offending heading, not just the first.
- The brief problems join the existing problem list, so specs, roadmap and brief are reported in one refusal rather than through separate paths.
- A complete brief still signs off, and the whole existing suite still passes.
...
- test_signoff_refuses_a_brief_missing_a_required_heading: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_signoff_refuses_a_heading_with_an_empty_body: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
...
- `uv run --with pytest python -m pytest factory/tests/test_gates.py -q -k "signoff or brief"`

## Reviewer focus

That the check reuses parse_sections rather than re-parsing, and that emptiness means no non-whitespace content.
