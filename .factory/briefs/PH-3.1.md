# Brief — PH-3.1: Nine sections, not one
...
Replace plan save's substring test for '## Surface Impact' with a parse_sections check over all nine planner sections, naming every missing one.

## Acceptance criteria

- plan save refuses a plan missing any of Problem, Scope / Non-goals, Acceptance Criteria, Technical Approach, Decisions, Surface Impact, Task Decomposition, Risks, Verify Plan.
- A section present with a whitespace-only body counts as missing, decided by parse_sections rather than a length threshold.
- The refusal names every missing section in one message, not the first one found.
- The check calls factory_lib.parse_sections; no second section matcher is added to plans.py.
- The rule binds at SAVE time and only to plan documents. plans/README.md, plans/assumptions.md and plans/deferrals.md are a readme and two ledgers, not plans, and are never checked.
- Archived plans under plans/completed/ are NOT rewritten to satisfy a rule invented after they shipped — FORGE-INIT-1 is missing eight sections and harness-v2-wedge five, and rewriting them would falsify the record. A test asserts an archived plan is left alone.
...
- test_plan_save_refuses_a_plan_missing_any_required_section: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_plan_save_names_every_missing_section: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_archived_plans_are_not_held_to_a_later_contract: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_plan_ledgers_are_not_plans: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
...
- `uv run --with pytest python -m pytest factory/tests/test_gates.py -q -k "plan_save or plan_sections"`

## Reviewer focus

That the rule binds where plans are WRITTEN, not everywhere plan-shaped files live. The first version of this task would have rewritten shipped, archived plans to satisfy a contract invented after they were saved.
