# Brief — PH-1.4: Doctor reports capture gaps without blocking
...
Report a brief or spec that predates this contract through forge doctor as advisory checks, so older projects are visible without being refused.

## Acceptance criteria

- forge doctor reports a brief missing required headings and names them.
- forge doctor reports each confirmed spec missing a required heading.
- Both checks are non-required, so a project captured before this contract is reported and never blocked.
- Neither check emits a line when there is nothing to report.
...
- test_doctor_reports_legacy_capture_without_blocking: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
...
- `uv run --with pytest python -m pytest factory/tests/test_gates.py -q -k "doctor"`

## Reviewer focus

That the checks are advisory and that a legacy fixture is reported rather than refused.
