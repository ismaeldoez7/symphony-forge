# Brief — DELEG-8: required tests become runner-owned proof
...
Replace opaque required_tests strings with proof objects. New decompositions validate each id, path and command; doctor reports legacy strings; stage done executes the declared command instead of inferring collection from source.

## Acceptance criteria

- new decompositions refuse string required_tests
- proof objects require non-empty id, safe repo-relative path and runnable command
- the command must bind both the declared path and id
- stage done refuses missing paths and failing exact commands
- doctor reports active legacy required_tests strings
- delegate briefs render exact required-test commands
...
- test_decomposition_refuses_string_required_tests: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_decomposition_refuses_malformed_required_test_objects: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_decomposition_refuses_required_test_command_without_path_or_id: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_stage_done_refuses_missing_required_test: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_doctor_reports_legacy_string_required_tests: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
...
- `uvx --with pytest python3 -m pytest factory/tests/test_gates.py -q -k "required_test or decomposition or stage_done or doctor or delegate"`
- `python3 factory/scripts/check_dual_runtime.py`

## Reviewer focus

validate the proof shape and execute the exact command; do not add a runner abstraction or source parser
