# Brief — DELEG-6: the delegation step is named where people look
...
The harness gets skipped partly because the delegation step was never printed anywhere. forge next names it, the forge skill maps the intents to it, and the decision record states why these are gates rather than instructions.

## Acceptance criteria

- forge next names the delegation step (brief, delegate, watch, verify) when a stage is active
- the forge skill maps "delegate this task" and "is Codex stuck?" to the new commands
- AGENTS.md states the delegation gate in one line and stays under its 110-line cap
...
- test_next_names_delegation_step
...
- `python3 -m pytest factory/tests/test_gates.py -q -k "next or agents"`
- `python3 factory/scripts/check_dual_runtime.py`

## Reviewer focus

the AGENTS.md line count gate must actually be run, not assumed; no policy block may land in AGENTS.md
