# Brief — PH-3.3: Retire authored waves
...
Stop anything authoring build_waves and delete its only reader in the same change, keeping the schema field optional so shipped artifacts still parse.

## Acceptance criteria

- build_waves is gone from factory/prompts/decomposer.md's output contract and from griller.md's checklist.
- factory/scripts/render_linear_task_graph.py is deleted in this same task, so the field never exists as a writer without a reader or the reverse.
- build_waves stays optional in factory/schemas/decomposition.json and the .factory/history/ artifacts carrying it still parse.
- Nothing else references render_linear_task_graph: a repo-wide search returns no callers, and check_dual_runtime stays green.
- The change cites decision 0021 derived-ordering, or restates the ordering claim if that record has not landed.
- docs/FACTORY.md stops advertising render_linear_task_graph.py. Deleting a script while a doc still tells people to run it is the same orphan the writer/reader rule forbids, pointing the other way.
- check_factory_scaffold.py stops requiring render_linear_task_graph.py. It lists the file as required scaffold, so deleting the script makes the scaffold check fail — the deletion is not atomic until every reference goes with it.

## Write scope — nothing outside this

- factory/prompts/decomposer.md
- factory/prompts/griller.md
- factory/scripts/render_linear_task_graph.py
- factory/scripts/check_factory_scaffold.py
- docs/FACTORY.md
...
- test_no_prompt_authors_build_waves: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
...
- `uv run --with pytest python -m pytest factory/tests/test_gates.py -q -k "waves or decomposer or history"`
- `python3 factory/scripts/check_dual_runtime.py`
- `python3 factory/scripts/check_agents_hygiene.py`

## Reviewer focus

That the writer and the reader die together. Split across tasks, one of them is an orphan by construction.
