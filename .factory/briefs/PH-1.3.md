# Brief — PH-1.3: Spec confirm refuses an incomplete spec
...
Require an H1 title plus Why, Behaviour and Acceptance criteria at forge spec confirm, before the grill check, while leaving spec save free-form.

## Acceptance criteria

- forge spec confirm refuses a spec missing an H1 title, ## Why, ## Behaviour or ## Acceptance criteria, and names what is missing.
- The heading check runs before require_grill, so an incomplete spec is refused without needing a grill first.
- forge spec save still accepts an incomplete draft.
- A heading present with an empty body is refused on the same terms as an absent one.
...
- test_spec_confirm_refuses_a_spec_missing_required_headings: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_spec_save_still_accepts_an_incomplete_draft: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
...
- `uv run --with pytest python -m pytest factory/tests/test_gates.py -q -k "spec"`

## Reviewer focus

That the casing matches the specs already on disk and that draft capture is genuinely unaffected.
