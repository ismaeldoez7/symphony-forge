# Brief — PH-2.1: The story and epic contracts, at the routes that author them
...
Extract check_story_contract(item, known_epics) and check_epic_contract(epic) in forge_cli/roadmap.py and call them from cmd_derive, cmd_import and cmd_add, leaving save_roadmap and the status routes on the weaker check_item.

## Acceptance criteria

- check_story_contract refuses a story that omits epic, story narrative, acceptance_criteria, skill, or depends_on, and names every missing field rather than the first.
- depends_on must be present even when empty: [] is accepted and a missing key is refused, so 'no dependencies' is stated rather than inferred from absence.
- check_story_contract refuses an epic id that is not in the known epics of the same payload or the stored roadmap.
- check_epic_contract requires id, title, objective, and source_refs that resolve to existing repo-relative paths; a source_ref pointing at a missing file is refused.
- cmd_derive, cmd_import and cmd_add all call both contracts; save_roadmap, mark_status, cmd_assign, cmd_link_spec and cmd_heal do not, and a legacy roadmap whose stories have no epic still loads, flips status, and heals.
- check_item keeps its existing type checks unchanged, so the two concerns stay separable.
...
- test_roadmap_authoring_refuses_an_incomplete_story: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_roadmap_authoring_requires_an_explicit_depends_on: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_epic_contract_refuses_an_unresolvable_source_ref: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_legacy_epicless_roadmap_still_loads_and_heals: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
...
- `uv run --with pytest python -m pytest factory/tests/test_gates.py -q -k "roadmap or epic or story_contract"`

## Reviewer focus

That validation lands only on the three authoring routes. cmd_heal runs at the post-merge moment a roadmap is least well-formed, and gating it would remove the recovery path exactly when it is needed.
