# Brief — PH-2.2: Two epic routes, and --epic required on add
...
Add forge roadmap epic add and forge roadmap set-epic, and require --epic on roadmap add, so that requiring a known epic does not lock the roadmap against its own backfill.

## Acceptance criteria

- roadmap epic add <id> --title --objective --source-ref (repeatable) writes an epic through save_roadmap's existing epics parameter, requires sign-off, and refuses a duplicate id.
- roadmap epic add does NOT require the epics-approved decision, because it appends one epic where import replaces the whole list; the asymmetry is stated in the command help.
- roadmap set-epic <story-key> --epic <id> writes the epic field on an existing story, refusing an unknown story key and an unknown epic id.
- roadmap add requires --epic and refuses an unknown epic id.
- roadmap add --no-spec --reason still produces a story with origin: adhoc and spec_debt_reason set, and that path has a test it did not have before.
- Duplicate story keys, self-dependencies and cycles remain refused at add.
- The two commands are registered in factory/scripts/forge.py, where every forge subcommand's argparse wiring lives; roadmap.py holds handlers only.
...
- test_epic_add_refuses_a_duplicate_id: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_set_epic_points_a_story_at_a_known_epic: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_roadmap_add_requires_a_known_epic: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_roadmap_add_no_spec_still_records_spec_debt: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
...
- `uv run --with pytest python -m pytest factory/tests/test_gates.py -q -k "epic_add or set_epic or roadmap_add or no_spec"`

## Reviewer focus

That the --no-spec carve-out survives intact. Decision 0014 puts authorization at plan save, and plans.py already refuses spec-debt stories, so removing the hatch would delete a designed path rather than close a hole.
