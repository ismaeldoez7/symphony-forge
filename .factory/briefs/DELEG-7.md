# Brief — DELEG-7: forge delegate owns companion execution
...
Replace shell-token inference with one executable boundary. forge delegate composes the brief, invokes the installed companion through a fixed argv, records successful launch evidence, and routes direct companion Bash calls back to the command.

## Acceptance criteria

- forge delegate launches the installed companion without a shell
- --print-only records no launch and cannot satisfy stage done
- successful launch evidence is bound to the stage, task, brief and argv
- stage done refuses without a fresh successful write launch
- direct companion Bash commands are routed to forge delegate
...
- docs/decisions/0018-delegation-gates.md
- AGENTS.md
- .claude/CLAUDE.md
- factory/tests/test_gates.py
- .review-notes.md
- WORKFLOW.md
- docs/decisions/0002-concurrency-one-task-per-branch.md
- docs/decisions/0007-stage-commit-loop.md
...
- test_delegate_records_ledger_entry: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_delegate_print_only_records_no_successful_launch: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_delegate_missing_companion_guides_doctor_fix: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
- test_hook_denies_unbriefed_write_delegation: `uvx --with pytest python3 -m pytest {path}::{id} -q -o junit_family=legacy --junitxml={report}` (factory/tests/test_gates.py)
...
- `uvx --with pytest python3 -m pytest factory/tests/test_gates.py -q -k "delegate or companion or stage_done or hook"`
- `python3 factory/scripts/check_dual_runtime.py`

## Reviewer focus

launch must use argv without a shell; print-only, read-only and stale evidence cannot satisfy stage close
