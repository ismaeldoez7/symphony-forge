...
- Current delta ID: `ee006954c4aca6347a255d7b8863b66583d531672fba50f2509e961a0da464a3`
...
```json
{
  ...
  "commands_run": [
    "FACTORY_TEST_CMD='UV_CACHE_DIR=/tmp/forge-coord-uv-cache UV_TOOL_DIR=/tmp/forge-coord-uv-tools uv run --python 3.11 --with pytest --with pytest-xdist --with psutil python -m pytest factory/tests -q -n auto --dist worksteal -p no:cacheprovider' UV_CACHE_DIR=/tmp/forge-coord-uv-cache UV_TOOL_DIR=/tmp/forge-coord-uv-tools uv run --python 3.11 --with pytest --with psutil python factory/scripts/verify.py -> structural clean; scaffold clean; 1042 passed, 3 skipped in 632.12s",
    "UV_CACHE_DIR=/tmp/forge-coord-uv-cache UV_TOOL_DIR=/tmp/forge-coord-uv-tools uv run --python 3.11 --with pytest --with psutil python -m pytest <exact 22-selector acceptance batch> -q -p no:cacheprovider -> 22 passed, 758 deselected in 75.23s",
    "UV_CACHE_DIR=/tmp/forge-coord-uv-cache UV_TOOL_DIR=/tmp/forge-coord-uv-tools uv run --python 3.11 --with pytest --with psutil python -m pytest factory/tests/test_gates.py -q -k '<three final boundary regressions>' -p no:cacheprovider -> 3 passed, 725 deselected in 11.59s",
    "UV_CACHE_DIR=/tmp/forge-coord-uv-cache UV_TOOL_DIR=/tmp/forge-coord-uv-tools uv run --python 3.11 --with pytest --with psutil python -m pytest factory/tests/test_native_launch.py factory/tests/test_native_setup.py factory/tests/test_worker_admission.py -q -p no:cacheprovider -> 71 passed in 37.08s",
    "UV_CACHE_DIR=/tmp/forge-coord-uv-cache UV_TOOL_DIR=/tmp/forge-coord-uv-tools uv run --python 3.11 --with pytest --with psutil python factory/scripts/check_encoding_hygiene.py -> encoding hygiene: OK; python3 -m py_compile and git diff --check clean",
    "Registered native worker PreToolUse protected-plan probes -> correlated deny before execution; plan SHA-256 remained 095c0f871e263c541f002f75e8388747718dc516b1bbee95c7bc998b413d34ec"
  ],
  "commit": "bcc3d89999c4220b98dc776a40c00961f21f94e6",
  "current_contract": {
    "c9": "Review inputs bind the full current approved plan, current task proof, and the exact current delta rendered into the detached provider dataset and review token.",
    ...
    "plan_sha256": "095c0f871e263c541f002f75e8388747718dc516b1bbee95c7bc998b413d34ec",
    "report": "/tmp/forge-root-bcc3d89-automated.json"
  },
  "current_evidence": {
    "artifacts": {
      "c8_source_check": {
  ...
  "pass_fail_summary": "Current host at bcc3d89: structural clean; scaffold clean; full suite 1,042 passed and 3 skipped; exact acceptance batch 22 passed; final boundary regressions 3 passed; native launch/setup/admission 71 passed; encoding, compilation, and diff checks clean.",
  "recorded_at": "2026-09-12T11:29:38+00:00",
  "remaining_gaps": [],
  "residual_risks": [],
  "reviewed_scope": [
    ".claude/CLAUDE.md",
  ...
  "status": "passed",
  "summary": "Current host proof at bcc3d89 passed structural and scaffold checks, all 1,042 runnable full-suite tests, the exact 22-selector acceptance batch, the three final boundary regressions, all 71 native launch/setup/admission tests, encoding hygiene, compilation, and diff checks. Correlated live C8 denial evidence is preserved unchanged.",
  "tests_added_or_updated": [
    "test_the_delegate_brief_carries_the_selected_current_findings",
    "test_recorder_holds_the_task_narrative_contract",
    "test_worker_symlink_entry_is_denied (delete and move cases)",
    "test_combined_review_refuses_incomplete_noncontiguous_missing_copied_or_mixed_output",
    "test_review_consumers_include_complete_approved_inputs",
    "test_review_brief_mints_run_id_and_lenses_echo_it",
    "test_task_start_creates_before_jit_with_approved_identity",
    "test_codex_exec_ban_matches_invocations_not_prose",
    "existing combined-review generation, selected-pointer, token, historical-delta, and closeout fixtures across the approved test modules"
  ]
}
```
